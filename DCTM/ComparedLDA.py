...
def compared_lda(read_directory1, read_directory2, read_filename, write_directory1, write_directory2, write_directory3):
    ...
    for i in range(file_number):
        ...
        lda = models.ldamodel.LdaModel(tf_corpus, num_topics=each_lt_number[i])
        
        #获取文档-潜在主题分布矩阵
        THETA = []
        for j in range(len(tf_corpus)):
            this_line = np.zeros(each_lt_number[i])
        ...
        #获取潜在主题-词汇分布矩阵
        PHAI = []
        raw_topics = lda.show_topics(topics=each_lt_number[i], formatted=False)
        for j in range(each_lt_number[i]):
...
if __name__ == '__main__':
    ...
    read_filename = root_directory + u'dataset/DCTM/mctrwctm_cn.txt'
    write_directory = root_directory + u'dataset/DCTM'
    write_directory1 = root_directory + u'dataset/DCTM/tf_corpus2'
    write_directory2 = root_directory + u'dataset/DCTM/doc_topic2'
    write_directory3 = root_directory + u'dataset/DCTM/topic_word2'
